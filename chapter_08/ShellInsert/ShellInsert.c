...
void ShellInsert(SqList *list) {
    ...
    for (dk = list->length / 2; dk >= 1; dk /= 2) {
        ...
        for (i = dk + 1; i <= list->length; i++) {
             ...
             */
             if (list->elemData[i] < list->elemData[i - dk]) {
                 // 将待排序元素 list[i] 复制到 list[0] 位置
                 list->elemData[0] = list->elemData[i];
                 // 第三层循环，进行插入排序
                 for (j = i - dk; j > 0 && list->elemData[0] < list->elemData[j]; j -= dk) {
                     // 如果 list[0] < list[j] 则将 list[j] 后移一位
                     list->elemData[j + dk] = list->elemData[j];
                 }
                 // 循环结束后 list[j + dk] 的位置就是 list[0] 待插入位置
                 list->elemData[j + dk] = list->elemData[0];
            }
        }
    }
}
